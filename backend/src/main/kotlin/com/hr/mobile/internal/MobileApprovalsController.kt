...
class MobileApprovalsController(
    ...
    fun getPendingApprovals(
        ...
        val pendingItems = buildDemoItems().filterNot { decidedItems.containsKey(it.id) }
    ...
    private fun buildDemoItems(): List<ApprovalItemDto> = listOf(
        ApprovalItemDto(
            id = "demo-leave-001",
    ...
    fun decideApproval(
        ...
        val tenantId = com.hr.tenancy.TenantContext.currentOrNull()?.id
            ?: UUID.fromString("01930000-0000-7000-8000-000000000001")

        val service = leaveApplicationService
        var eventHandledByService = false
        if (runCatching { UUID.fromString(id) }.isSuccess && service != null) {
            ...
            runCatching {
                ...
                eventHandledByService = true
            }
        }

        // If not dispatched to an actual leave entity, publish event for demo items so push notifications fire
        if (!eventHandledByService) {
            val demoTarget = buildDemoItems().find { it.id == id }
            if (demoTarget != null && demoTarget.type == "LEAVE_APPLICATION") {
                val appUuid = runCatching { UUID.fromString(id) }.getOrElse { UUID.nameUUIDFromBytes(id.toByteArray()) }
                if (decision == "APPROVE") {
                    eventPublisher?.publishEvent(
                        LeaveApprovedEvent(
                            tenantId = tenantId,
                            applicationId = appUuid,
                            employeeId = demoTarget.requesterId,
                            approverId = approverId,
                            leaveTypeName = demoTarget.leaveDetails?.leaveTypeName ?: "Leave",
                            startDate = demoTarget.leaveDetails?.startDate ?: LocalDate.now(),
                            endDate = demoTarget.leaveDetails?.endDate ?: LocalDate.now(),
                            totalDays = BigDecimal.valueOf(demoTarget.leaveDetails?.workingDays ?: 1.0),
                    ...
                    eventPublisher?.publishEvent(
                        LeaveRejectedEvent(
                            ...
                            endDate = demoTarget.leaveDetails?.endDate ?: LocalDate.now(),
                            totalDays = BigDecimal.valueOf(demoTarget.leaveDetails?.workingDays ?: 1.0),
                            reason = request.remarks ?: "Rejected by manager",
                        ),
                    )
                }
            }
